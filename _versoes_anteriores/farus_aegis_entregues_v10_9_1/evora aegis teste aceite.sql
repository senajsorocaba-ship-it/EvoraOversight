...
insert into auth.users (email) values ('autoridade@gab.gov.br') on conflict do nothing;
...
where t.slug='gab-a' and us.email='autoridade@gab.gov.br';
\set ON_ERROR_STOP off
insert into auth.users (email) values ('assessor@gab.gov.br') on conflict do nothing;
insert into usuarios (tenant_id, auth_user_id, nome, email, papel, ativo)
