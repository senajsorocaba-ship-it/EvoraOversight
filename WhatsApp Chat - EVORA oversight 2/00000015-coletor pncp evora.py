...
CNPJ_ORGAO  = ""                 # opcional: CNPJ do órgão (Prefeitura) p/ filtrar
...
def buscar_contratos():
    ...
    params = {
        ...
        # "codigoMunicipioIbge": IBGE_MUNIC,
        ...
        def g(*chaves):
            for k in chaves:
                if isinstance(it, dict) and it.get(k) not in (None, ""):
                    return it[k]
            return ""
        # filtro por município (quando a API não filtrou)
        municipio = str(g("municipioNome", "municipio", "nomeMunicipio"))
