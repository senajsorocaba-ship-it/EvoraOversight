# Manual Supremo Évora V10.9 — Edição Consolidada
...
# Manual Supremo Évora V10.9 — VOLUME 1: Visão & Estratégia
...
| Freio Humano: o que ele trava e o que não trava `[v10.9.1]` | 9 |
| Selo de Maturidade — a régua de confiança do produto `[v10.9.1]` | 10 |
| Decisões de Lançamento — Distribuição, Recorte e Contexto `[v10.9.1]` | 10 |
| Porta de Suporte VRX — Três Zonas de Alçada `[v10.9.1]` | 11 |
...
| Anexo 21 — Registro de Correções da Consolidação v10.9.1 `[v10.9.1]` | 119 |
...
- **Acervo de informação verificada (FARUS)** - banco central que só ingere dado com fonte verificável, 24/7 (ver CVI, DA-0002).
...
## Freio Humano: o que ele trava e o que não trava `[v10.9.1 — corrigido]`
...
## Selo de Maturidade — a régua de confiança do produto `[v10.9.1 — novo]`
...
4. **Dados preservados.** O acervo verificado do Évora é um ativo do negócio e um registro histórico do setor — não um custo a minimizar. Ver a política de retenção revisada no FARUS e no Anexo 17.
...
## FARUS — Radar e Acervo de Informação Verificada
...
7. **Alimentar o FARUS** (o repositório central — com selo do CVI);
...
**Escala:** começa em Sorocaba (MVP), expande à Região Metropolitana (fase seguinte) e, com a mesma arquitetura, cobre os **645 municípios do Estado de São Paulo**. Cada novo município é um conector + configuração — o FARUS coleta **uma vez** e serve a **todos os tenants** daquela cidade (vantagem estrutural de custo).

**No MVP:** ativa-se **apenas o RMS-001 Sorocaba**, focado no que serve à tenant zero: **nomeações/exonerações, contratos/licitações e pautas** — o restante do conteúdo fica arquivado no FARUS para uso futuro.
...
1. **Primeiro o banco (instantâneo):** a Bia consulta o **FARUS** — tudo o que os coletores já ingeriram (Diário Oficial, jornais, PNCP) responde em segundos, com fonte e data.
...
**Objetivo.** Gerenciar num só lugar todas as fontes que o Évora monitora, permitindo o repositório crescer sem limite, com governança e rastreabilidade total. É o painel operacional do que os módulos COM (Coletores Oficiais Municipais) e FARUS consomem.
...
6. **Documentos** — repositório de arquivos: PDFs, contratos, pareceres, ofícios, anexos, com pesquisa OCR. Distinto do Banco de Dados (FARUS): FARUS guarda *informação verificada* (dado estruturado com selo CVI); Documentos guarda *arquivos* (o PDF em si). Um indexa fatos; o outro, papéis.
...
## Porta de Suporte VRX — Três Zonas de Alçada `[v10.9.1 — novo]`
...
## Decisões de Lançamento — Distribuição, Recorte e Contexto `[v10.9.1 — novo]`
...
Vale para as **fontes de pesquisa diária** (AIM-g / FARUS) e para o **Veritas — Acervo Legal-Normativo**. O Évora nunca fica preso a um conjunto fixo de fontes: cresce com a necessidade do gabinete, e só enxuga sob controle.
...
A **VRX Sistemas Inteligentes** é o **departamento de manutenção dos sistemas Évora** — marca de serviço da operadora, não pessoa jurídica autônoma; a execução pode caber a técnico próprio ou a prestador terceirizado (ver Anexo 18). Na Fase 1, antes de hardware de Sentinela, a manutenção concentra-se no software do Núcleo Fundador.
...
| Zero-Knowledge | Desenho em que o provedor não consegue ler o conteúdo **depositado no cofre**. Aplica-se ao Aegis, **não à plataforma inteira** — o motor precisa ler o acervo e o perfil para produzir o briefing. |
...
# Manual Supremo Évora V10.9 — VOLUME 2: Manual dos Agentes
...
# Manual Supremo Évora V10.9 — VOLUME 3: Produtos em Detalhe
...
O **Aegis** é o cofre de dados sensíveis (N5), com desenho **zero-knowledge para o conteúdo nele depositado**. Fora do cofre, a plataforma processa os dados necessários à sua finalidade, sob isolamento por cliente e registro de acesso.

> **Correção da v10.9.1.1.** A redação anterior afirmava que o provedor não conseguiria ler o conteúdo do cliente, sem qualificação. **Não pode valer para a plataforma inteira**: o motor precisa ler o Diário Oficial, comparar contratos e cruzar com o perfil para produzir o briefing. Prometer zero-knowledge para tudo é promessa que o produto não cumpre.

| Camada | Provedor lê? |
| --- | --- |
| Acervo FARUS | sim — é público e precisa ser processado |
| Perfil do gabinete | sim — o motor precisa dele |
| Briefings gerados | sim — produzidos pelo próprio sistema |
| **Cofre Aegis (N5)** | **não** |

A chave-mestra é fragmentada por esquema de quórum, de modo que nenhum indivíduo isolado detém acesso total.


### Chave-mestra e ausência da autoridade

| Mecanismo | Regra |
| --- | --- |
| Chave-mestra (Aegis) | Fragmentada por esquema 2-de-3 (ex.: autoridade + advogado + custódia notarial). |
| Ausência da autoridade (Sigma One) | **Exclusivo da tenant.** 5 guardiões cadastrados, 3 abrem; advogado obrigatório na composição mínima; sem fallback; lacre após o uso. Acionado somente nas hipóteses de ausência da autoridade — **não trata de sucessão do fundador**. |
...
A ingestão segue o pipeline do COM (verificar → baixar → extrair → classificar → FARUS), com o selo do CVI. Novas normas entram continuamente (a Porta de Fontes permite acréscimo por qualquer assessor; exclusão só com alçada).
...
# Manual Supremo Évora V10.9 — VOLUME 4: Anexos
...
## FARUS — Radar e Acervo de Informação Verificada `[v10.9.1 — novo]`

> **Em resumo:** o FARUS é o radar, o acervo e a memória informacional permanente do Évora. Começa onde o Genesis manda começar, mas pode descobrir novos territórios. Guarda o que encontrou, acompanha o que continua relevante, e entrega inteligência ao Briefing Matinal e aos agentes autorizados.

### Nome

**FARUS** — *Radar e Acervo de Informação Verificada*. **Não é sigla nem acrônimo.** O nome remete a *pharus/pharos* — farol: vigilância permanente, alcance além do ponto onde está fixado, alerta quando enxerga algo relevante.

O descritor emprega **"Informação"**, e não "Mídias", porque o componente ingere Diário Oficial, contratações públicas e precedente legislativo — que não são mídia. Descritor estreito induziria modelagem apenas de imprensa.

> **Nota de marca:** nome evocativo é mais defensável no INPI que nome descritivo, que encontra resistência por ninguém poder monopolizar palavras comuns do ramo. FARUS protege o nome; o descritor apenas explica.

### Regra pétrea

**Nunca inventar. Nunca mentir. Nunca apresentar como fato aquilo que não foi comprovado.**

Quando não souber, o FARUS declara que não foi possível verificar. Quando as fontes divergirem, apresenta a divergência. **É preferível admitir a lacuna a preenchê-la com invenção.** A regra é estrutural: não depende de pressão, urgência ou importância da resposta.

### Classificação da informação

| Estado | Significado |
| --- | --- |
| **Confirmado** | Sustentado por fonte primária ou evidência robusta |
| **Corroborado** | Confirmado por duas ou mais fontes independentes compatíveis |
| **Indicado** | Fonte secundária ou parcial — recomenda confirmação |
| **Inferência** | Conclusão lógica derivada de fatos — **nunca apresentada como fato original** |
| **Hipótese** | Possibilidade não comprovada |
| **Conflitante** | Fontes incompatíveis entre si |
| **Desatualizado** | Foi verdadeiro; há evidência posterior que pode ter alterado o estado |
| **Não verificado** | Evidência insuficiente |

### Ciclo de vida da fonte

**descoberta → observada → configurada**

O FARUS descobre, cataloga e monitora sozinho. **Nenhuma fonte descoberta autonomamente entra no Atlas nem alimenta afirmação de briefing sem liberação humana com alçada.** A fonte descoberta entra em estado de observação, aparece no Catálogo com a origem da descoberta, e só então passa por avaliação de pessoa.

> **A trava não atrasa o alerta.** O aviso chega imediatamente. O que se impede é que a descoberta vire **afirmação** sem passar por alguém. Reconcilia o Radar Emergente com a decisão D6 do Anexo 16.

### Camada compartilhada — e seu limite

O FARUS coleta as fontes de um município **uma única vez** e serve **todos os tenants** daquele território. É a economia de escala que sustenta o negócio: mais tenants, mais território monitorado, maior cobertura para todos.

**Mas o compartilhamento tem fronteira.** Todo registro carrega **origem da aquisição**:

| Origem | Compartilhável |
| --- | --- |
| Coleta territorial programada | **sim** — acervo público |
| Descoberta autônoma do radar | **sim** — acervo público |
| Pesquisa disparada por interesse de tenant | **não** — contexto do tenant |
| Pesquisa sob demanda de tenant | **não** — contexto do tenant |

> **Sem esse campo, a diferenciação de planos vira vazamento entre clientes.** É requisito de segurança, não de produto.

### Faseamento aprovado

| Fase | Conteúdo | Quando |
| --- | --- | --- |
| **F1 — Memória** | Tabelas · ingestão das fontes cadastradas · selo CVI · deduplicação por hash · retenção indeterminada · leitura compartilhada por município | **Antes do beta** |
| **F2 — Catálogo** | Catálogo de Fontes com alçada · estados da fonte · territórios manuais | Durante o beta |
| **F3 — Radar** | Descoberta autônoma · radar emergente · alerta de menção fora do território | Após o beta |
| **F4 — Antecipação** | Detecção de interesse · pesquisa de precedentes · monitoramento por assunto | Com receita |
| **F5 — Cruzamento** | Verificação societária e poderes de representação | Após parecer jurídico |

**Apenas a F1 condiciona o lançamento.** As demais são arquitetura de destino: aprovadas, documentadas, não construídas.

### Transição de cargo do tenant

O cargo define o universo informacional. Quando o cargo muda, mudam a autoridade, os órgãos fiscalizados, as normas aplicáveis e as fontes.

**Três campos no perfil:** `cargo_em_exercicio` governa o radar ativo · `cargo_eleito` dispara preparação · `data_posse` define a inversão.

**Entre a eleição e a posse**, o FARUS constrói o acervo do novo cargo **em segundo plano**, enquanto o briefing continua servindo o mandato vigente. Na posse, o peso inverte.

**Três travas:**

1. **Acrescentar, não substituir.** O acervo do território anterior permanece — é base eleitoral, é histórico pelo qual a autoridade segue respondendo, e a memória não se destrói ao alterar o radar.
2. **A transição é evento governado.** Exige confirmação humana com alçada, prévia do que muda, registro na trilha imutável e aviso à equipe.
3. **O Veritas troca junto.** O acervo legal é indexado por cargo. Sistema que cite Regimento Municipal a um deputado federal está dando informação errada com aparência de certeza.

### Níveis de produto

| Nível | Radar | Cliente natural |
| --- | --- | --- |
| **Municipal** | município + limítrofes confirmados | Vereador · Prefeito |
| **Estadual** | estado + capital | Deputado estadual |
| **Nacional** | federal + estado de base + Brasília | Deputado federal · Senador |

**Todo nível contém o inferior.** A curadoria dos sete blocos permanece idêntica em todos: **nível superior significa radar mais largo com a mesma disciplina**, jamais volume maior. Briefing que despeja duzentos itens não é lido por ninguém.
...
### Dados coletados no cadastro `[v10.9.1]`

| Campo | Nível | Finalidade |
| --- | --- | --- |
| Nome completo | N4 | identificação |
| **CPF** | **N5** | identificação unívoca · exigido em contrato e nota fiscal |
| **Data de nascimento** | **N5** | conferência de identidade · desempate de homônimo |
| E-mail funcional | N4 | acesso e entrega |
| ~~RG~~ | — | **removido em 02/09/2026** |

**Fundamento da remoção do RG.** O Manual prevê que dado N5 viva no **Aegis**, que está em L3/L4 e não existe construído. Coletar documento adicional sem o cofre previsto é assumir passivo sem a proteção correspondente.

> **Princípio adotado:** dado não coletado não vaza, não precisa ser protegido e não entra em incidente. Se o CPF resolve, o RG é passivo sem contrapartida.

**CPF e data de nascimento são exibidos mascarados.** Ambos figuram entre os campos mais usados em fraude de identidade. Guardar é necessário; exibir não é.
...
### Consolidação do Motor Oficial (achado de 25/07/2026) `[v10.9.1 — novo]`
...
Dois vereadores da mesma cidade compartilham o Atlas e **nunca** compartilham uma linha de perfil. O isolamento vereador × vereador (diferencial de venda, v10.2) permanece intacto — e o custo cai: o FARUS coleta a cidade uma vez e serve todos os tenants dela.
...
| **Veritas** | Componente | Acervo jurídico-normativo e FARUS — Radar e Acervo de Informação Verificada. Marca visual própria identificada em 22/07/2026; não constava do inventário até a v10.9.1 | A DEPOSITAR |
...
### Natureza da VRX `[v10.9.1]`

**VRX Sistemas Inteligentes** deixa de figurar como entidade societária e passa a **marca de serviço da operadora** — é o **departamento de manutenção** dos sistemas Évora. **O nome permanece inalterado.**

A execução técnica pode caber a **técnico próprio ou a prestador terceirizado**, indiferentemente. A distinção que importa não é o vínculo — é o **acesso**.

**Travas aplicáveis igualmente a próprio e a terceirizado:**

| Trava | Razão |
| --- | --- |
| Confidencialidade e cessão de PI assinadas **antes do primeiro acesso** | Todo código sem cessão expressa gera dúvida sobre titularidade |
| Alçada limitada à zona verde | Zona amarela exige confirmação da operadora; a vermelha ninguém alcança |
| **Identidade nominal na trilha** | Credencial compartilhada "do suporte" destrói a auditoria: o registro passa a dizer *o que* foi feito, nunca *por quem* |
| Revogação no mesmo dia | Desligamento, fim de contrato ou troca de equipe |
| Ambiente de teste por padrão | Dado real exige autorização expressa e específica |

**A Porta de Suporte VRX permanece inalterada** — três zonas, com a vermelha recusando sempre, inclusive ao fundador. A governança não depende de a VRX ser empresa ou departamento: **a porta não distingue quem pede, distingue o que é pedido.**

**Declaração ao cliente.** Para o cliente, "VRX" soa como empresa. O contrato comercial deve informar que o suporte pode ser prestado por terceiro sob confidencialidade. Descoberta tardia custa confiança; declaração prévia não custa nada.

### Marca nova `[v10.9.1]`

**FARUS** entra no inventário como marca a depositar. Origem: *pharus/pharos* — farol. Ver a seção FARUS no corpo do Manual.

### Holding `[v10.9.1]`

A titularidade das marcas caberá à **Evotec Participações Ltda.**, holding licenciadora que não opera. Enquanto não houver ato formal de transferência, **as marcas e o código permanecem do fundador, pessoa física** — a holding não detém o que o contrato social declara que ela detém.
...
| Retenção de dado pessoal | P11 (Anexo 3) | Reconcilia a nova política de retenção indeterminada (FARUS) com a LGPD para dado de cidadão | Dra. Íria | Alta |
...
## Anexo 21 — Registro de Correções da Consolidação v10.8 `[v10.8 — histórico]`
...
Fila de registro aberta em 22/07/2026, emitida na v10.8. Oito itens acumulados por decisão deliberada — a fila existe para evitar dez versões do Manual em duas semanas —, mais um nono item apurado durante a própria emissão.
...
## Anexo 22 — Registro de Correções da Consolidação v10.9 `[v10.9.1 — novo]`

> **Em resumo:** o que mudou da v10.8 para a v10.9.1, por quê, e o que continua pendente. Emitido a partir da fila aberta em 02/09/2026, com 22 itens.

### Origem

Fila de registro aberta em 02/09/2026. Vinte e dois itens acumulados: oito decisões do fundador, seis apurações e oito correções herdadas ou de vocabulário. Emitida nesta versão.

### Decisões do fundador incorporadas

| # | Decisão | Data |
| --- | --- | --- |
| 1 | **FARUS — Radar e Acervo de Informação Verificada** substitui "Veritas-Dados" | 02/09/2026 |
| 2 | **Transição de cargo:** cargo em exercício em primeiro plano até a posse; cargo eleito em segundo plano desde o resultado | 02/09/2026 |
| 3 | **Três níveis de produto:** Municipal, Estadual, Nacional | 02/09/2026 |
| 4 | **Faseamento do FARUS:** apenas a F1 antes do beta; F2 a F5 como arquitetura de destino | 02/09/2026 |
| 5 | **Radar com trava:** descoberta autônoma alerta na hora, mas a fonte entra como *observada* | 02/09/2026 |
| 6 | **Dados coletados:** nome, CPF, data de nascimento e e-mail funcional. **RG removido** | 02/09/2026 |
| 7 | **VRX Sistemas Inteligentes** passa a departamento de manutenção; nome inalterado | 02/09/2026 |
| 8 | **Evotec Participações Ltda.** — holding licenciadora; tipo societário Ltda | 02/09/2026 |

### Apurações registradas

**A1 — Quinta ocorrência de "documento diz, disco não tem".** O Manual descreve o repositório central como oitavo moat desde a v7.7. O banco instalado tem onze tabelas e nenhuma guarda o material coletado; o coletor de imprensa gravava em arquivo local sobrescrito a cada execução. O acervo declarado não existia.

**A2 — Sexta ocorrência, em sentido inverso.** Relatou-se como ausente a Frente 6 do Anexo 3 (perguntas 19 a 24). **A Frente 6 existe**; a cópia em disco é que estava incompleta. **Lição:** a divergência entre documento e disco ocorre nos dois sentidos — verificar a fonte antes de declarar lacuna.

**A3 — Sigma × Aegis.** Registrou-se confusão de vocabulário: **Aegis** é o cofre de dados; **Sigma One** é o protocolo de sucessão por quórum. Sigma não guarda arquivo. Ambos permanecem em L3/L4 e **não existem construídos** — enquanto não houver Aegis, não há cofre para dado N5.

### Correções de nomenclatura

Registrar, junto a Verix e Khodex, os nomes avaliados e descartados para o repositório central, **com o motivo**, para impedir reproposição futura:

| Formulação | Motivo do descarte |
| --- | --- |
| Veritas-Dados | Ambiguidade com o Veritas legal — confundiu na prática |
| Acervo | Sugere coleção estável; o componente é fluxo |
| Pulso | Colide com o bloco 04 do briefing |
| Radar de Mídias / RCIV | Colide com o bloco 02; "Mídias" estreito demais |
| Repositório Central de Informações Verificadas (RCIV) | Substituída |
| Repositório Central de Informações Dinâmicas (RCI) | Sigla não fechava |
| Fonte de Arquivo e Repositório de Informações e Mídias Verificadas | Acrônimo não fechava; "Mídias" estreito |
| Fonte de Arquivo e Repositório Inteligente de Mídias | Daria FARIM; "Mídias" estreito |
| FARUS Verified / Verifield | "Verifield" não é palavra em inglês |

### Migração de banco registrada
...
Decorre da correção do Freio Humano na v10.8: o briefing é entregue direto às 06:45; o registro é de **ciência**, não de aprovação prévia.

### O que permanece pendente

| Pendência | Situação |
| --- | --- |
| Teto operacional para territórios ilimitados | Decisão adiada — morde na F3 |
| Pesquisa por interesse nos Termos de Uso | Antes do primeiro cliente pagante |
| Aegis A0 | **construído e testado** em 02/09/2026 — ver Anexo 23 |
| Aegis A1–A3 e Sigma One | ainda não construídos |
| Parecer jurídico | Brief v2, 37 perguntas — 14 travam o lançamento |
| Revisão de segurança independente | Condição de entrada do primeiro cliente |

---

## Anexo 23 — Estado Real de Construção `[v10.9.1.1 — novo]`

> **Em resumo:** o que existe **no banco**, verificado por execução, em 02 de setembro de 2026. Este anexo existe porque o padrão "documento diz, disco não tem" já se repetiu seis vezes — e a única cura é registrar o que foi executado, não o que foi planejado.

### Construído e testado em PostgreSQL 16

| Componente | Arquivo | Evidência |
| --- | --- | --- |
| Schema base | `evora_schema_mvp_v1.sql` | 11 tabelas |
| Isolamento por tenant | `evora_rls_mvp_v1.sql` | 13 políticas · RLS forçado |
| Teste de aceite base | `evora_teste_aceite_v1.sql` | 5 testes |
| **Hook de token** | `evora_token_hook.sql` | **testado em 02/09** — injeta identidade; **falha fechada** para usuário inativo ou inexistente |
| **FARUS F1** | `evora_farus_f1.sql` | 4 tabelas · 4 políticas · RLS forçado |
| **Teste do FARUS** | `evora_farus_teste_aceite.sql` | **10 de 10** |
| **Aegis A0** | `evora_aegis_a0.sql` | 2 tabelas · cofre sem política de leitura |
| **Teste do Aegis** | `evora_aegis_teste_aceite.sql` | **10 de 10** |

### O que o FARUS F1 entrega

Acervo compartilhado por município, com deduplicação por hash, retenção indeterminada e o campo `origem_aquisicao` que separa acervo público de contexto do tenant.

**Provado por teste:** dois gabinetes da mesma cidade **compartilham** o acervo público e **não veem** a pesquisa privada um do outro. É a economia de escala do negócio sem custo de isolamento.

**Travas impostas pelo banco, não pelo programa:** fonte não vira `configurada` sem liberação humana registrada (decisão D6) · acervo público não pode ter dono · nada é `confirmado` sem verificador registrado · acervo público não pode ser excluído · conteúdo repetido não entra duas vezes.

### O que o Aegis A0 entrega

**A verificação que precedeu a construção:** o schema não tinha coluna de CPF, o motor não usava dado sensível e o front-end apenas mascarava. **O cofre nasceu antes do primeiro dado sensível existir** — situação que não se repete.

| Trava | Como funciona |
| --- | --- |
| Aplicação não lê o cofre | **Não existe política de leitura.** A ausência é a proteção, não esquecimento |
| Leitura exige motivo declarado | Única porta é a função `aegis_ler` |
| Acesso gravado antes de devolver | Leitura sem registro não existe |
| Registro imutável | Nem alteração nem exclusão |
| Alçada | Apenas autoridade e chefe de gabinete abrem |
| Máscara fora do conteúdo | A tela nunca precisa abrir o cofre |
| Isolamento entre gabinetes | O vizinho não vê nem a forma mascarada |

### Defeito corrigido durante a construção

`farus_hash` foi escrita como `IMMUTABLE` chamando `unaccent`, que é `STABLE`. O PostgreSQL aceita, e **corrompe silenciosamente qualquer índice construído sobre a função**. Substituída por `translate` com tabela fixa de acentos. Retestada.

### O que ainda NÃO foi verificado

| Pendência | Consequência |
| --- | --- |
| Nada rodou no Supabase de produção | Tudo acima foi testado em ambiente local |
| Hook não ativado no painel | Sem isso, todo token nasce sem tenant e o banco nega tudo |
| Backup nunca restaurado | Cópia não testada é considerada inexistente |
| Revisão de segurança independente | **Condição de entrada do primeiro cliente** |
| Cabeçalhos web, limite de tentativas | Dependem da hospedagem |

> **Registro de método.** Durante os testes, duas falhas pareceram defeito do código e eram do próprio teste: inserção com nomes de coluna inexistentes, e teste de RLS rodado como superusuário (que ignora RLS por natureza). **Vale ao programador:** teste de isolamento exige transação explícita e troca de papel; sem isso, o teste mente.
